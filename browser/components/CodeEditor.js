...
export default class CodeEditor extends React.Component {
    ...
    handlePaste(editor, e) {
        ...
        const isInLinkTag = (editor) => {
            ...
            const prevChar = editor.getRange({ line: startCursor.line, ch: startCursor.ch - 2 }, { line: startCursor.line, ch: startCursor.ch })
            const endCursor = editor.getCursor('end')
            const nextChar = editor.getRange({ line: endCursor.line, ch: endCursor.ch }, { line: endCursor.line, ch: endCursor.ch + 1 })
            return prevChar === '](' && nextChar === ')'
        }
        if (dataTransferItem && dataTransferItem.type && dataTransferItem.type.match('image')) {
            const { storageKey, noteKey } = this.props
