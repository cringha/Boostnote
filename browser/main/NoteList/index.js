...
const config = ConfigManager.get();
                ...
                if ( blog && blog.blogId ) {
                    ...
                    menu.append(new MenuItem({
                        label: fetchBlogLabel,
                        click: () => this.updateMarkdown.bind(this)(note)
                    }));
  ...
  save (note) {
    ...
    updateMarkdown() {
        ...
        this.pendingUpdated = setTimeout(() => {
            this.updateMarkdownFromServer()
        }, 200)
    }
  ...
  confirmPublish(  note) {
    ...
    confirmUpdated(note) {
        const buttonIndex = dialog.showMessageBox(remote.getCurrentWindow(), {
            type: 'warning',
            message: i18n.__('Updated Succeeded'),
            detail: `${note.title} is Updated`,
